...
<main>
                ...
                <div class="modal-body">
                    <div class="row" style="direction: rtl">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" id="_date">
                        </div>
                        <div class="col-md-4">
                            <label>
                                <b>فى تمام الساعة</b>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control" id="_time_pmam">
                                <option value="مساءاَ">مساءاَ</option>
                                <option value="صباحاَ">صباحا</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label>
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control" id="_details">
                        </div>
                    </div>
                </div>
    ...
    <div id="edits_buttons">
        <ul class="list-unstyled list-inline">
            <li>
                <button type="button" data-toggle="modal" data-target="#myModal" class="btn btn-default">ادخال
                    البيانات
                </button>
            </li>
            ...
            <li>
                ...
                <a href="{{route('allExternalPPFromView')}}" class="btn btn-primary">العودة</a>
            </li>
        </ul>
    </div>
    <div id="menu">
...
<script>
    ...
    function addDetails() {
        ...
        let _date = $('#_date').val();
        let _time = $('#_time').val();
        let _time_pmam = $('#_time_pmam').val();
        let _details = $('#_details').val();
        ...
        $('#violation_date').text(_date).css({color: 'red'});
        $('#violation_time').text(_time + ' ' + _time_pmam).css({color: 'red'});
        $('#violation_details').text(_details).css({color: 'red'});
    ...
    function getAgeFromSSN($ssn) {
        try {
            ...
            let $birth_date = moment([parseInt($year), parseInt($x[3].concat($x[4])), parseInt($x[5].concat($x[6]))]);
            let $_birth_date = moment([parseInt($year), parseInt($x[3].concat($x[4])), parseInt($x[5].concat($x[6]))], 'YYYYMMDD');

            let a = $birth_date;
            let b = moment([{{date('Y')}}, {{date('m')}}, {{date('d')}}], 'YYYYMMDD');
    ...
    $('#submit_edit').on('click', function () {
        ...
        $.ajax({
            url: '{{route('handleCreateExtrenalPPFrom')}}',
            type: 'POST',
            data: {
                _token: '{{csrf_token()}}',
                html: html,
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    Swal.fire({
                        type: 'success',
                        text: 'تم الاضافة بنجاح هل تريد الاضافة مرة اخري ؟',
                        title: 'نجاح',
...
<script>
    ...
    function addInput() {
        // positionX = 0
        // positionY = 0
        // $('body').append('<div class="drag ui-draggable ui-draggable-handle">' +
        //     '<input type="text" class="form-control" style="z-index: 999;width: 250px;position: absolute"/>' +
        //     '<div/>');
        // setTimeout(() => {
        //     $('.drag').draggable().click(function () {
        //         $(this).draggable({disabled: false});
