...
@section('content')
    <div class="skeleton-nav--center">
        <div class="container-fluid">
            <div class="row">
                @include('pages.violation.punishment.all.templates.header')
                @include('pages.violation.punishment.all.templates.content')
            </div>
        </div>
    </div>
@endsection
        ...
        function remove(id) {
            Swal.fire({
                ...
                preConfirm: function (allow) {
                    if (allow) {
                        $.ajax({
                            url: "{{route('handleDeletePunishmentRules')}}",
                            method: "delete",
                            data: {
                                _token: '{{csrf_token()}}',
                                id: id
