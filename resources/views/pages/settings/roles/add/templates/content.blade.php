                    ...
                    <div class="col-md-4">
                        ...
                        <span class="input input--isao">
                            <input class="input__field input__field--isao" name="title" autocomplete="off" type="text"
                                   value="{{old('title')}}" placeholder="{{__('violation.name_type')}}"/>
                            ...
                            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                @foreach($permissions as $k => $permission)
                                    @if(!auth()->user()->checkFullAccess())
                                        @if( $k != 'roles')
                                            <a class="nav-item nav-link text-uppercase @if($loop->first) active @endif" id="nav-{{$k}}-tab"
                                               data-toggle="tab"
                                                ...
                                                @foreach($permission as $p_item)
                                                    <div class="col-md-3 mb-1">
                                                        ...
                                                        <input class="styled-checkbox" name="permissions[]"
                                                               ...
                                                               value="{{$p_item->id}}">
                                                        <label for="styled-checkbox-{{$p_item->id}}">{{app()->getLocale() =='ar'? $p_item->name_ar : $p_item->name}}</label>
