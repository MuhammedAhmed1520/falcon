...
@section('content')
    <div class="skeleton-nav--center">
        <div class="container-fluid">
            <div class="row">
                @include('pages.officeAgent.reports.getOfficeApprovedReportView.templates.header')
                @include('pages.officeAgent.reports.getOfficeApprovedReportView.templates.content')
            </div>
        </div>
    </div>
@endsection
...
@section('scripts')
    ...
    <script>
        function printTrigger(){
           ...
           let comp2 = $('select[name="classification_degree_id"] option:selected').text();
            $('#company_name').text(comp)
            $('#classification_degree').text(comp2)
            window.print();
        }
