...
@section('content')
    <div class="skeleton-nav--center">
        <div class="container-fluid">
            <div class="row">
                @include('pages.officeAgent.offices.getManagerUserView.templates.header')
                @include('pages.officeAgent.offices.getManagerUserView.templates.content')
            </div>
        </div>
    </div>
@endsection
...
@section('scripts')
    ...
    <script>
        ...
        function changeState(id) {
            let start_date = $('#violation_'+id).find('input[name="start_date"]').val()
            let end_date = $('#violation_'+id).find('input[name="end_date"]').val()
            let allow = confirm('{{__('violation.are_sure')}}');

            if (allow) {
                $.ajax({
                    url: "{{route('handleManagerUserRequest')}}",
                    method: "post",
                    data: {
                        _token: '{{csrf_token()}}',
                        id: id,
