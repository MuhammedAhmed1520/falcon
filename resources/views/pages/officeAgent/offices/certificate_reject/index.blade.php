...
@section('content')
    <div class="skeleton-nav--center">
        <div class="container-fluid">
            <div class="row">
                @include('pages.officeAgent.offices.certificate_reject.templates.header')
                @include('pages.officeAgent.offices.certificate_reject.templates.content')
            </div>
        </div>
    </div>
@endsection
...
@section('scripts')
    ...
    <script>
        ...
        function deleteStopCertify(id) {
            let check = confirm('هل انت متأكد ')
            if (check) {
                $.ajax({
                    url: '{{route('deleteStopCertify',['id'=>request()->id])}}',
                    type: 'post',
                    data: {
                        id: id,
                        _token: '{{csrf_token()}}'
