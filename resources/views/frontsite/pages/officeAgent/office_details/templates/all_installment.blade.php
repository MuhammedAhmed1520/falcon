    ...
    <div class="col-md-12">
        ...
        @if(checkSendAllowed($office_agent))
            <button type="button" class="btn btn-danger float-right" data-toggle="modal"
                    onclick="resetToAddInsatllment()" id="installment_add_modal_btn" name="add_device"
...
<div class="row mt-4">
    <div class="col-md-12">
        ...
        @if(checkSendAllowed($office_agent))
            <button type="button" class="btn btn-danger float-right" data-toggle="modal" name="add_device_attachment"
        ...
        <table class="table table-ressponsive" id="installment_attachment_table">
            ...
            @foreach($office_agent->devices_attachments as $devices_attachment)
                <tr id="all_installment_attachment_{{$devices_attachment->id}}">
                    ...
                    <td>
                        @if(checkSendAllowed($office_agent))
                            <button class="btn btn-danger" name="delete_device_attachment" onclick="deleteInstallmentFile('{{$devices_attachment->id
                        }}')">
                                <i class="icon icon-trash mr-0"></i>
                            </button>
                        @endif
